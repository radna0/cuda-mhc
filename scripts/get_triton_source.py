...
app = modal.App("debug-triton-full-return-fixed")
...
if __name__ == "__main__":
    with app.run():
        src = get_kernel_source.remote()
        with open("logs/matmul_ogs_source.py", "w") as f:
            f.write(src)
        print(f"Saved source to logs/matmul_ogs_source.py ({len(src)} bytes)")
