...
def patch_gpt_oss():
    ...
    class Mxfp4GptOssExperts_Training(torch.autograd.Function):
        ...
        def backward(ctx, grad_token):
            ...
            # grad_token might be [B, S, D] or [N, D]
            ...
            hidden_size = self_class.hidden_size
            intermediate_size = self_class.intermediate_size

            # =================================================================
            # CRITICAL FIX #1: Correct scatter indexing for backward pass
            # Forward scatter: output[scatter_dst] = expert_out[scatter_src] * gamma
            # Backward: grad_exp = grad_token[scatter_dst] * gamma
            # =================================================================
            
            # Debug: Check tensor shapes and index bounds
            n_tokens = grad_token.size(0)
            
            # Clamp indices to valid range to prevent out of bounds errors
            # This can happen if gradient checkpointing recomputes with different shapes
            scatter_dst_safe = scatter_dst.clamp(0, n_tokens - 1)
            # gather_src_safe = gather_src.clamp(0, n_tokens - 1) # Will be used later
            
            # 1) Gather gradients from token positions where forward scattered TO
            grad_exp = grad_token.index_select(0, scatter_dst_safe)  # [N_exp, hidden_size]
            grad_exp = grad_exp * gamma.unsqueeze(-1)
            
            # =================================================================
            # MXFP4 STE Backward with Correct Dimensions
            # Forward: [hidden] -> gate_up -> [2*inter] -> swiglu -> [inter] -> down -> [hidden]
            # Backward: [hidden] <- gate_up^T <- [2*inter] <- swiglu' <- [inter] <- down^T <- [hidden]
            # 
            # Since MXFP4 can't be transposed, we use dimension-preserving STE:
            # - Transform gradient shapes to match forward activations
            # - Apply activation gradient using saved pre_act
            # - Transform back to input shape
            # =================================================================
            
            # 2) grad_exp has shape [N_exp, hidden_size]
            #    Need to transform to [N_exp, intermediate_size] for down_proj backward
            #    STE approximation: interpolate dimensions
            grad_intermediate = torch.nn.functional.interpolate(
                grad_exp.unsqueeze(1),  # [N_exp, 1, hidden_size]
                size=intermediate_size,
                mode='linear',
                align_corners=False
            ).squeeze(1)  # [N_exp, intermediate_size]
            
            # 3) Transform to [N_exp, 2*intermediate_size] for swiglu backward
            #    Duplicate to match gate+up structure
            grad_pre_act = torch.zeros(
                grad_intermediate.shape[0], 
                2 * intermediate_size, 
                device=grad_intermediate.device, 
                dtype=grad_intermediate.dtype
            )
            grad_pre_act[..., ::2] = grad_intermediate   # gate component
            grad_pre_act[..., 1::2] = grad_intermediate  # up component
            
            # 4) Apply SwiGLU activation gradient using saved pre_act
            #    swiglu_backward: computes element-wise gradient through activation
            dx_pre_act = swiglu_torch_backward(pre_act, alpha, limit, grad_pre_act)
            # dx_pre_act has shape [N_exp, 2*intermediate_size]
            
            # 5) Transform from [N_exp, 2*intermediate_size] back to [N_exp, hidden_size]
            #    Average gate and up gradients, then interpolate
            dx_gate = dx_pre_act[..., ::2]   # [N_exp, intermediate_size]
            dx_up = dx_pre_act[..., 1::2]    # [N_exp, intermediate_size]
            dx_combined = (dx_gate + dx_up) * 0.5  # [N_exp, intermediate_size]
            
            dx_exp = torch.nn.functional.interpolate(
                dx_combined.unsqueeze(1),  # [N_exp, 1, intermediate_size]
                size=hidden_size,
                mode='linear',
                align_corners=False
            ).squeeze(1)  # [N_exp, hidden_size]
            
            # Apply scaling to maintain gradient magnitude
            # Approximate weight norm ratio
            scale = (hidden_size / intermediate_size) ** 0.5
            dx_exp = dx_exp * scale
            
            # =================================================================
            # CRITICAL FIX #2: Correct gather indexing for backward pass
            # Forward gather: expert_in[gather_dst] = token[gather_src]
            # Backward: grad_token[gather_src] += grad_exp[corresponding positions]
            # =================================================================
            
            # 6) Scatter gradients back to token positions
            gather_src_safe = gather_src.clamp(0, n_tokens - 1)
            dx_token = torch.zeros_like(grad_token)
            dx_token.index_add_(0, gather_src_safe, dx_exp.to(dx_token.dtype))  # Use clamped indices and cast to correct dtype
...
class GptOssMLP(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Check if we have MXFP4 experts (Triton kernel path)
        # MXFP4 experts have gate_up_proj as a Triton tensor, not gate_up_proj_blocks
        is_mxfp4 = hasattr(self.experts, "gate_up_proj") and not hasattr(self.experts, "gate_up_projs")
        
        if is_mxfp4:
            # MXFP4 path: use Triton routing kernel
            # Import routing function from triton_kernels
            try:
                from triton_kernels.routing import routing
                routing = torch.compiler.disable(routing)
            except ImportError:
                # Fallback: try from patched mxfp4
                from transformers.integrations.mxfp4 import mlp_forward
                return mlp_forward(self, hidden_states)
            
            # Compute router logits
            hidden_flat = hidden_states.reshape(-1, self.router.hidden_dim)
            router_logits = nn.functional.linear(hidden_flat, self.router.linear.weight, self.router.linear.bias)
            
            # Use Triton routing to get proper routing data
            with torch_cuda_device(router_logits.device):
                routing_data, gather_idx, scatter_idx = routing(router_logits, self.router.top_k)
            
            # Call experts with proper Triton routing arguments
            routed_out = self.experts(hidden_flat, routing_data, gather_idx, scatter_idx)
            routed_out = routed_out.reshape(bsz, -1, self.router.hidden_dim)
            return routed_out, router_logits
        else:
            # Non-MXFP4 path: use standard router_indices/routing_weights
