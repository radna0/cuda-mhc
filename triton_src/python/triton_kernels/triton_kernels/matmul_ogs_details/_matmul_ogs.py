...
def _matmul_ogs(
    ...
    if is_w_microscaled:
        ...
        tl.static_assert(SWIZZLE_MX_VALUE == "HOPPER_VALUE" or SWIZZLE_MX_VALUE == "BLACKWELL_VALUE" or SWIZZLE_MX_VALUE is None, "Only Hopper and Blackwell swizzling is supported for values")
    else:
    ...
    if is_w_microscaled:
        if SWIZZLE_MX_VALUE == "HOPPER_VALUE" or SWIZZLE_MX_VALUE == "BLACKWELL_VALUE":
    ...
    if B is not None:
        ...
        if pid_k == 0:
            bias = tl.load(BPtrs, mask=mask_n, other=0.0).to(tl.float32)
        else:
